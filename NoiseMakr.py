#!/usr/bin/env python3
"""NoiseMakr CMD - command-line Python port of NoiseMakr2 20VT VAG logic.

Profiles and byte modifications are reproduced from NoiseMakr2 v1.2.0.1:
  Low, Medium, High
...
class NoiseMakrError(RuntimeError):
    pass
...
def find_kfzwmn(data: bytes) -> list[int]:
    """Return map starts using the exact NoiseMakr2 condition: match + 30."""
    matches: list[int] = []
...
def choose_detected(name: str, matches: list[int]) -> int:
    if not matches:
        raise NoiseMakrError(f"{name} was not found automatically.")
    if len(matches) > 1:
        rendered = ", ".join(f"0x{x:06X}" for x in matches)
        raise NoiseMakrError(
...
def ensure_range(data: bytearray, address: int, offsets: Iterable[int], map_name: str) -> None:
    if address < 0:
        raise NoiseMakrError(f"{map_name} address cannot be negative.")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CMD port of NoiseMakr2 Pops & Bangs logic for VAG 20VT BIN files."
    )
...
def main() -> int:
    ...
    print("=" * 72)
    print(f" {APP_NAME} v{APP_VERSION} - NoiseMakr2 logic port")
    print("=" * 72)

    input_path: Path = args.input.expanduser().resolve()
    if not input_path.is_file():
        raise NoiseMakrError(f"Input file not found: {input_path}")
    ...
    if output_path == input_path:
        raise NoiseMakrError("Output path must be different from the input path.")
